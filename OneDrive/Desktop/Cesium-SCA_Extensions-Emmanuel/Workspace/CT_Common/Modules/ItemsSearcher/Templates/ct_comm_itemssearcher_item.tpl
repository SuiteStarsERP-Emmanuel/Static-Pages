{{#if isItemSelected}}
  <a class="itemssearcher-item-results" data-hashtag="{{model._url}}"
    data-touchpoint="home">
    <div class="itemssearcher-item-results-image">
      <img data-loader="false" class="typeahead-image"
        src="{{resizeImage model._thumbnail.url 'thumbnail'}}"
        alt="{{model._thumbnail.altimagetext}}">
    </div>
...
{{else}}
  {{#if hasResults}}
    <div class="itemssearcher-item-all-results">
      {{translate 'See all results'}}<i class="ct-icon ct-icon-arrow-right"></i>
